...
Generated by CritiqueQuest`;
  ...
  private async processBulkImportContent(content: any): Promise<{ cases: CaseStudy[], collectionInfo: any }> {
    ...
    if (content.cases && Array.isArray(content.cases)) {
      ...
      throw new Error('Invalid format: Expected collection with cases array or single case study');
    }
  }
  ...
  private async exportBulkToJSON(caseStudies: CaseStudy[], filename: string): Promise<string> {
    const collection = {
      ...
      collection_info: {
        ...
        description: `Collection of ${caseStudies.length} case studies`,
        total_cases: caseStudies.length,
        domains: Array.from(new Set(caseStudies.map(c => c.domain))),
        complexities: Array.from(new Set(caseStudies.map(c => c.complexity))),
      },
