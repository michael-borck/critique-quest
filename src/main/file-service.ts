...
<head>
    ...
    <title>${esc(caseStudy.title)}</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>${esc(caseStudy.title)}</h1>
        <div class="metadata">
            <strong>Domain:</strong> ${esc(caseStudy.domain)} |
            <strong>Complexity:</strong> ${esc(caseStudy.complexity)} |
            <strong>Type:</strong> ${esc(caseStudy.scenario_type)}
        </div>
        <div class="metadata">
            <strong>Word Count:</strong> ${esc(caseStudy.word_count)} words
        </div>
        <div class="tags">
            ${caseStudy.tags.map(tag => `<span class="tag">${esc(tag)}</span>`).join('')}
        </div>
    </div>

    <div class="section">
        <h2>Case Study</h2>
        <div class="content">${esc(caseStudy.content)}</div>
    </div>

    ${caseStudy.questions ? `
    <div class="section">
        <h2>Analysis Questions</h2>
        <div class="content">${esc(caseStudy.questions)}</div>
    </div>
    ` : ''}
    ...
    <div class="section">
        <h2>Model Answers</h2>
        <div class="content">${esc(caseStudy.answers)}</div>
    </div>
    ` : ''}
...
Generated by CritiqueQuest`;
  ...
  async importCaseFromURL(url: string): Promise<CaseStudy> {
    try {
      // Validate URL and block private/loopback hosts (SSRF defence)
      ...
      const caseStudy: CaseStudy = {
        ...
        word_count: countWords(String(content.content)),
        usage_count: 0,
  ...
  async importBulkCasesFromURL(url: string): Promise<ImportResult> {
    try {
      // Validate URL and block private/loopback hosts (SSRF defence)
      await assertPublicUrl(url);
  ...
  async importBulkCasesFromFile(content: string): Promise<ImportResult> {
    try {
  ...
  private async processBulkImportContent(content: unknown): Promise<ImportResult> {
    const data = content as Record<string, unknown>;
    
    // Check if it's a Bundle format (v2.0)
    if (data.bundle_info && (data.bundle_info as Record<string, unknown>).version === '2.0') {
      return this.processBundleImport(content as unknown as Bundle);
    }
    ...
    if (data.cases && Array.isArray(data.cases)) {
      ...
      const collectionInfo = {
        title: (data.collection_info as Record<string, unknown>)?.title as string || 'Imported Collection',
        description: (data.collection_info as Record<string, unknown>)?.description as string || `Collection of ${data.cases.length} case studies`,
        total_cases: data.cases.length,
        exported_by: data.exported_by as string || 'Unknown',
        exported_at: data.exported_at as string || new Date().toISOString(),
        version: data.version as string || '1.0',
      };

      const importedCases: CaseStudy[] = [];
      
      for (const rawCaseData of data.cases) {
        const caseData = rawCaseData as Record<string, unknown>;
        ...
        const cleanedCase: CaseStudy = {
          ...
          complexity: ['Beginner', 'Intermediate', 'Advanced'].includes(caseData.complexity as string) 
            ? caseData.complexity as 'Beginner' | 'Intermediate' | 'Advanced'
            : 'Intermediate',
          scenario_type: ['Problem-solving', 'Decision-making', 'Ethical Dilemma', 'Strategic Planning'].includes(caseData.scenario_type as string)
            ? caseData.scenario_type as 'Problem-solving' | 'Decision-making' | 'Ethical Dilemma' | 'Strategic Planning'
            : 'Problem-solving',
          ...
          word_count: countWords(String(caseData.content)),
          usage_count: 0,
          created_date: new Date().toISOString(),
          modified_date: new Date().toISOString(),
      ...
      const singleCase = await this.importCaseFromContent(data);
      return {
        cases: [singleCase],
        collectionInfo: {
  ...
  private async processBundleImport(bundle: Bundle): Promise<ImportResult> {
    const importedCases: CaseStudy[] = [];
  ...
  private async importCaseFromContent(content: Record<string, unknown>): Promise<CaseStudy> {
    ...
    return {
      title: String(content.title),
      domain: String(content.domain || 'General'),
      complexity: ['Beginner', 'Intermediate', 'Advanced'].includes(content.complexity as string) 
        ? content.complexity as 'Beginner' | 'Intermediate' | 'Advanced'
        : 'Intermediate',
      scenario_type: ['Problem-solving', 'Decision-making', 'Ethical Dilemma', 'Strategic Planning'].includes(content.scenario_type as string)
        ? content.scenario_type as 'Problem-solving' | 'Decision-making' | 'Ethical Dilemma' | 'Strategic Planning'
        : 'Problem-solving',
      content: String(content.content),
      questions: String(content.questions || ''),
      answers: content.answers ? String(content.answers) : undefined,
